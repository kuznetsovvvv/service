...
class API {
 ...
 public:
  API(crow::App<crow::CORSHandler>& App, const std::string& db_name,
      const std::string& dsn)
      : app(App), postgres(db_name, dsn) {
    setupRoutes(postgres);
  }
  ...
  void setupRoutes(DB& postgres) {
    ...
    CROW_ROUTE(app, "/api/v1/login")
        .methods(
            crow::HTTPMethod::PATCH)([&postgres](const crow::request& req) {
          string admin_phone = "+89814599088";
          string admin_password = "misha2005";
          string phone;
          string password;
          try {
            auto json = crow::json::load(req.body);
          ...
          try {
            if (phone == admin_phone && password == admin_password) {
              return crow::response(
                  crow::status::OK,
                  crow::json::wvalue({{"message", "Hello, Admin!"}}));
            } else {
        ...
        .methods(crow::HTTPMethod::GET)([&postgres](const crow::request& req) {
          ...
          try {
            ...
            if (product_info.has_value()) {
              crow::json::wvalue responce_data{};
              responce_data["product_id"] =
                  product_info.value().get_product_id();
    ...
    // передать заказ курьеру courier_action, соответственно изменив статус
    // заказа update_order_status
    CROW_ROUTE(app, "/api/v1/order")
        .methods(
            crow::HTTPMethod::PATCH)([&postgres](const crow::request& req) {
          int order_id;
          ...
          try {
            if (action == "transfer") {
              ...
              db::courier_action courier_action1("deliver", order_id,
                                                 courier_id);
              auto add = postgres.add_courier_action(courier_action1);
              auto succes = postgres.update_order_status(order1);
              if (!succes || add == false) {
    ...
    CROW_ROUTE(app, "/api/v1/cancelorder")
        .methods(
          ...
          try {
            ...
            auto update = postgres.cancel_order(order1, user_id);
            if (update == false) {
              return crow::response(
                  crow::status::BAD_REQUEST,
