...
public:
    ...
    courier(const int& id) : courier_id(id), age(0), gender(""), phone(""), active("") {}

    courier( const int& age, const string& gender,
             const string& phone, const string& active) :
         age(age), gender(gender), phone(phone), active(active) {}
...
public:
    ...
    courier_action(const string& action, const int& order_id, const int& courier_id) :
...
public:
    ...
    user(const int& age, const string& gender,
         const string& phone) :
         age(age), gender(gender), phone(phone) {}

    user(const int& id, const int& age, const string& gender,
         const string& phone) :
...
public:
   order() : order_id(0), creation_time(""), time(""), delivery_address(""), status("") {}

   order(const int& id): order_id(id), creation_time(""), time(""), delivery_address(""), status("") {}
   ...
   order(const int& id,const string& delivery_address,const string& status) :
         order_id(id),status(status),delivery_address(delivery_address) {}
